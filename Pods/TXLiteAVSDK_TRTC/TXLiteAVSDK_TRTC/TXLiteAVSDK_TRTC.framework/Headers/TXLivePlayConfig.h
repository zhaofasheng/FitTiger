 ...
 */
LITEAV_EXPORT @interface TXLivePlayConfig : NSObject
...
///【字段含义】视频渲染对象回调的视频格式，默认值：kCVPixelFormatType_420YpCbCr8BiPlanarFullRange。
///【特别说明】支持：kCVPixelFormatType_420YpCbCr8Planar 和
...
///【字段含义】是否开启回声消除，从 9.9 版本开始，本设置项已不再支持，由 SDK 内部自动抉择，无需再调用，请忽略此设置项。
...
///【字段含义】RTMP 传输通道的类型，从 9.9 版本开始，本设置项已不再支持，由 SDK 内部自动抉择，无需再调用，请忽略此设置项。
@property(nonatomic, assign) int rtmpChannelType;

///【字段含义】视频缓存目录，从 9.9 版本开始，本设置项已不再支持，由 SDK 内部自动抉择，无需再调用，请忽略此设置项。
@property NSString *cacheFolderPath;

///【字段含义】最多缓存文件个数，从 9.9 版本开始，本设置项已不再支持，由 SDK 内部自动抉择，无需再调用，请忽略此设置项。
