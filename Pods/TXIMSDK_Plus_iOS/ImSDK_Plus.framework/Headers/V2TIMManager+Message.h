 ...
 * @param message 消息对象，消息需满足三个条件：1、消息发送前需设置 supportMessageExtension 为 YES，2、消息必须是发送成功的状态，3、消息不能是社群（Community）和直播群（AVChatRoom）消息。
...
/**
 *  5.21 标记单聊会话已读（待废弃接口，请使用 cleanConversationUnreadMessageCount 接口）
 *
 *  @note 请注意：
...
/**
 *  5.22 标记群组会话已读（待废弃接口，请使用 cleanConversationUnreadMessageCount 接口）
 *
 *  @note 请注意：
 *  - 该接口调用成功后，自己的未读数会清 0。
...
/**
 *  5.23 标记所有会话为已读（待废弃接口，请使用 cleanConversationUnreadMessageCount 接口）
 */
...
/// 离线推送设置 vivo 手机推送消息的类型，0：运营消息，1：系统消息。默认取值为 1。（应用配置接入 vivo 推送可以根据需要修改设置）
