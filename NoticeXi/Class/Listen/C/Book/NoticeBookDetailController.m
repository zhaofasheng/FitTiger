...
@interface NoticeBookDetailController ()<NoticewTextDelegate,NoticeNewVoiceListClickDelegate,LCActionSheetDelegate,NoticeManagerUserDelegate,NoticeRecordDelegate,NewSendTextDelegate>
...
@implementation NoticeBookDetailController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navigationItem.title = [NoticeTools getLocalStrWith:@"book.ct"];
    self.detialView = [[NoticeMovieDetail alloc] initWithFrame:CGRectMake(0, 0, DR_SCREEN_WIDTH, 240)];
    self.detialView.isBook = YES;
    if (self.book.book_id) {
        self.book.bookId = self.book.book_id;
    }
    if (!self.bookId) {
        self.detialView.book = self.book;
        self.detialView.frame = CGRectMake(0, 0, DR_SCREEN_WIDTH, 160+self.detialView.infoL.frame.size.height+(self.book.isMoreLines?30:0)+16);
    }
...
- (void)backToPageAction{
    ...
    [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[NoticeBookBaseController class]]) {//返回到指定界面
            pushVC = obj;
...
- (void)requestde{
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:[NSString stringWithFormat:@"resources/2/%@",self.bookId?self.bookId: self.book.bookId] Accept:@"application/vnd.shengxi.v3.2+json" isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        if (success) {
            self.book = [NoticeBook mj_objectWithKeyValues:dict[@"data"]];
            self.detialView.book = self.book;
            if (self.book.subscription) {
                self.book.subscription_id = [NSString stringWithFormat:@"%@",self.book.subscription[@"id"]];
                self.book.subscription_type = [NSString stringWithFormat:@"%@",self.book.subscription[@"subscription_type"]];
            }
            if (self.needRefresh) {
                self.detialView.scorL.text = self.book.book_score;
            }
            if (self.needRefresh) {
                self.needRefresh = NO;
            }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    label.text = [NSString stringWithFormat:@"书评 %@",self.num.intValue?self.num:@""];
...
- (void)createRefesh{
    ...
    __weak NoticeBookDetailController *ctl = self;
...
- (void)request{
    ...
    if (self.isDown) {
        ...
        url = [NSString stringWithFormat:@"resources/2/%@/comment",self.book.bookId];
    }else{
        url = [NSString stringWithFormat:@"resources/2/%@/comment?sortType=1&lastId=%@",self.book.bookId,self.lastId];
    }
    ...
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:url Accept:@"application/vnd.shengxi.v5.0.0+json" isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        ...
        if (success) {
            ...
            if (self.dataArr.count) {
                ...
                self.queshenView.titleStr = [NoticeTools getLocalStrWith:@"book.nocom"];
