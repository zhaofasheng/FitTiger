//
//  NoticeMovieDetailViewController.m
//  NoticeXi
//
...
#import "NoticeMovieViewController.h"
...
@interface NoticeMovieDetailViewController ()<NoticewTextDelegate,NoticeNewVoiceListClickDelegate,LCActionSheetDelegate,NoticeManagerUserDelegate,NoticeRecordDelegate,NewSendTextDelegate>
...
@implementation NoticeMovieDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navigationItem.title = [NoticeTools getLocalStrWith:@"movie.moviect"];
    self.detialView = [[NoticeMovieDetail alloc] initWithFrame:CGRectMake(0, 0, DR_SCREEN_WIDTH, 149)];
    
    self.detialView.movice = self.movie;
    self.detialView.frame = CGRectMake(0, 0, DR_SCREEN_WIDTH, 160+self.detialView.infoL.frame.size.height+(self.movie.isMoreLines?30:0)+16);
...
- (void)backToPageAction{
    ...
    [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[NoticeMovieBaseController class]]) {//返回到指定界面
            pushVC = obj;
...
- (void)requestde{
    ...
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:[NSString stringWithFormat:@"resources/1/%@",self.movie.movie_id?self.movie.movie_id:self.movie.movieListId] Accept:@"application/vnd.shengxi.v3.2+json" isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        if (success) {
            self.movie = [NoticeMovie mj_objectWithKeyValues:dict[@"data"]];
            if (self.movie.subscription) {
                self.movie.subscription_id = [NSString stringWithFormat:@"%@",self.movie.subscription[@"id"]];
                self.movie.subscription_type = [NSString stringWithFormat:@"%@",self.movie.subscription[@"subscription_type"]];
            }
            if (self.needRefresh) {
                self.detialView.scorL.text = self.movie.movie_score;
            }
            self.needRefresh = NO;
            self.detialView.movice = self.movie;
            self.detialView.frame = CGRectMake(0, 0, DR_SCREEN_WIDTH, 160+self.detialView.infoL.frame.size.height+(self.movie.isMoreLines?30:0)+16);
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    label.text = [NSString stringWithFormat:@"影评 %@",self.num.intValue?self.num:@""];
...
- (void)requestDetail{
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:[NSString stringWithFormat:@"resources/1/%@",self.movieId] Accept:@"application/vnd.shengxi.v3.2+json" isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        if (success) {
            self.movie = [NoticeMovie mj_objectWithKeyValues:dict[@"data"]];
            self.detialView.movice = self.movie;
            self.detialView.frame = CGRectMake(0, 0, DR_SCREEN_WIDTH, 160+self.detialView.infoL.frame.size.height+(self.movie.isMoreLines?30:0)+16);
...
- (void)createRefesh{
    ...
    __weak NoticeMovieDetailViewController *ctl = self;
...
- (void)request{
    NSString *url = nil;
    if (!self.movie.movie_id && self.movie.movieListId) {
        self.movie.movie_id = self.movie.movieListId;
    }
    if (self.isDown) {
        [self reSetPlayerData];
        url = [NSString stringWithFormat:@"resources/1/%@/comment",self.movie.movie_id];
    }else{
        url = [NSString stringWithFormat:@"resources/1/%@/comment?sortType=1&lastId=%@",self.movie.movie_id,self.lastId];
    }
    [[DRNetWorking shareInstance] requestNoNeedLoginWithPath:url Accept:@"application/vnd.shengxi.v5.0.0+json" isPost:NO parmaer:nil page:0 success:^(NSDictionary *dict, BOOL success) {
        ...
        if (success) {
            ...
            if (self.dataArr.count) {
                ...
                self.queshenView.titleStr = [NoticeTools getLocalStrWith:@"movie.nokanguo"];
