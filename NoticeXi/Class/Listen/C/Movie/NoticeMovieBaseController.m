...
#import "NoticeSearchMovieViewController.h"
...
@interface NoticeMovieBaseController ()<NoticeNewVoiceListClickDelegate,NoticeRecordDelegate,NewSendTextDelegate>
@property (nonatomic, strong) NSString *lastId;
@property (nonatomic, strong) NoticeVoiceListModel *hsVoiceM;
@property (nonatomic, strong) NoticeVoiceListModel *oldModel;
@property (nonatomic, strong) NoticeHotMovieList *hotListView;
@property (nonatomic, assign) BOOL isDown;  //YES  下拉
@end

@implementation NoticeMovieBaseController
...
- (void)viewDidLoad {
    ...
    self.hotListView = [[NoticeHotMovieList alloc] initWithFrame:CGRectMake(0, 0, DR_SCREEN_WIDTH,66+155+45+10+10)];
    ...
    self.navigationItem.title = [NoticeTools getLocalStrWith:@"main.movie"];
...
- (void)createRefesh{
    ...
    __weak NoticeMovieBaseController *ctl = self;
...
- (void)request{
    ...
    if (self.isDown) {
        ...
        url = [NSString stringWithFormat:@"club/1/voices?lastId=%@",self.lastId];
    }
...
- (void)searchClick{
    NoticeSearchMovieViewController *ctl = [[NoticeSearchMovieViewController alloc] init];
    [self.navigationController pushViewController:ctl animated:YES];
}
