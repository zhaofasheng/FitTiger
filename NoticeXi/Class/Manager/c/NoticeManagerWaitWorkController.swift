//
//  NoticeManagerWaitWorkController.swift
//  NoticeXi
//
...
import UIKit
class NoticeManagerWaitWorkController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(workRequest), name: NSNotification.Name(rawValue:"waitWork"), object: nil)
    }
    
    @objc func workRequest() {
        self.isDwon = true
        self.request()
    }
    /*请求数据*/
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if model.resource_type == "4" || model.resource_type == "126" || model.resource_type == "147"{
            ...
            self.showHUD()
            DRNetWorking.shareInstance()?.requestNoNeedLogin(withPath: "mass/" + model.chatTeamM.mass_id, accept: "application/vnd.shengxi.v5.5.2+json", isPost: false, parmaer: nil, page: 0, success: { [weak self] (dict, success) in
                self?.hideHUD()
                if success {
                    
                    let teamM = NoticeGroupListModel.mj_object(withKeyValues: dict!["data"])
                    teamM?.is_join = "1"
