//
//  NoticeWorkedAlreadyController.swift
//  NoticeXi
//
...
class NoticeWorkedAlreadyController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(workedRequest), name: NSNotification.Name(rawValue:"worked"), object: nil)
    }
    
    @objc func workedRequest() {
        self.isDwon = true
        self.request()
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if model.resource_type == "4" || model.resource_type == "126"{
            ...
            self.showHUD()
            DRNetWorking.shareInstance()?.requestNoNeedLogin(withPath: "podcast/" + model.podModel.podcast_id, accept: "application/vnd.shengxi.v4.9.7+json", isPost: false, parmaer: nil, page: 0, success: { [weak self] (dict, success) in
                self?.hideHUD()
                if success {
