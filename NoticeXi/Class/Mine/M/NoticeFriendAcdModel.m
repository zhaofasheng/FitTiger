...
@implementation NoticeFriendAcdModel
...
- (void)setCreated_at:(NSString *)created_at{
    _created_at = [NSString stringWithFormat:@"%@%@",[NoticeTools timeDataAppointFormatterWithTime:created_at.intValue appointStr:@"yyyy-MM-dd"],[NoticeTools getLocalStrWith:@"like.likeyou"]];
}

- (void)setLevel:(NSString *)level{
    _level = level;
    self.levelImgName = [NSString stringWithFormat:@"Image_leave%d",level.intValue>21?21:level.intValue];
    self.smallLevelImgName = [NSString stringWithFormat:@"Image_smalleave%d",level.intValue>21?21:level.intValue];
    if (level.intValue == 0) {
