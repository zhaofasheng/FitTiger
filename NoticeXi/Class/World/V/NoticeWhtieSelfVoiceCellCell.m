//
//  NoticeWhtieSelfVoiceCellCell.m
//  NoticeXi
//
...
@implementation NoticeWhtieSelfVoiceCellCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _timeL.textColor = [[UIColor colorWithHexString:@"#5C5F66"] colorWithAlphaComponent:1];
        ...
        [self.otherMoreBtn setImage:UIImageNamed(@"Image_sangedianw") forState:UIControlStateNormal];
        ...
        [self.hsButton setBackgroundImage:UIImageNamed(@"Image_newclickhsw") forState:UIControlStateNormal];
        ...
        self.dialNumL.textColor = [UIColor colorWithHexString:@"#5C5F66"];
        ...
        [self.sendBGBtn setBackgroundImage:UIImageNamed(@"Ima_sendbgnw") forState:UIControlStateNormal];
        ...
        self.bgL.textColor = [UIColor colorWithHexString:@"#5C5F66"];
        ...
        self.comImagV.image = UIImageNamed(@"Image_pinglunvw");
        ...
        self.comNumL.textColor = [UIColor colorWithHexString:@"#5C5F66"];
        ...
        [btn setBackgroundImage:UIImageNamed(@"Image_moreNeww") forState:UIControlStateNormal];
...
- (UIButton *)hadAddBtn{
    if (!_hadAddBtn) {
        ...
        [_hadAddBtn addTarget:self action:@selector(noJoinClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setVoiceM:(NoticeVoiceListModel *)voiceM{
    ...
    if (!voiceM.resource && voiceM.isSelf){
        ...
        self.numImageView.frame = CGRectMake(self.backView.frame.size.width-53, self.timeL.frame.origin.y+19, 12, 12);
        ...
        self.numImageView.image = UIImageNamed(_voiceM.content_type.intValue==1?@"Image_rednumw":@"Image_looknumw");
    ...
    [self.sendBGBtn setBackgroundImage:UIImageNamed(_voiceM.is_collected.intValue?@"Image_songbg": @"Ima_sendbgnw") forState:UIControlStateNormal];
    //书影音类型
...
- (void)noJoinClick{
    ...
    [[DRNetWorking shareInstance] requestWithDeletePath:[NSString stringWithFormat:@"user/%@/albumVoice/%@/%@",[NoticeTools getuserId],self.albumId,self.voiceM.voice_id] Accept:@"application/vnd.shengxi.v3.8+json" parmaer:nil page:0 success:^(NSDictionary * _Nullable dict, BOOL success) {
        if (success) {
            self.voiceM.hasCurrentJion = NO;
            self.moreBtn.hidden = NO;
            self.hadAddBtn.hidden = YES;
        }
...
- (UIView *)statusView{
    if (!_statusView) {
        ...
        _statusL.textColor = [UIColor colorWithHexString:@"#8A8F99"];
        _statusView.hidden = YES;
...
- (void)cancelPri{
    ...
    [nav.topViewController showHUD];
    NSMutableDictionary *parm = [NSMutableDictionary new];
    [parm setObject:self.voiceM.is_private.integerValue ? @"0" : @"1" forKey:@"privateStatus"];
    [[DRNetWorking shareInstance] requestWithPatchPath:[NSString stringWithFormat:@"users/%@/voices/%@",[[NoticeSaveModel getUserInfo] user_id],self.voiceM.voice_id] Accept:nil parmaer:parm page:0 success:^(NSDictionary *dict, BOOL success) {
        if (success) {
