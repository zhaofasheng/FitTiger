...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,NoticeAlbumChoiceDelegate> {
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        _imagePickerVc.navigationBar.barTintColor = [UIColor colorWithHexString:@"#F7F8FC"];
        _imagePickerVc.navigationBar.tintColor = [UIColor colorWithHexString:@"#F7F8FC"];
...
#pragma 选择单张照片时候的导航栏走这里
- (void)viewDidLoad {
    [super viewDidLoad];
    self.isFirstAppear = YES;
    
    self.albumView = [[NoticeAlbumView alloc] initWithFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT, DR_SCREEN_WIDTH, DR_SCREEN_HEIGHT-NAVIGATION_BAR_HEIGHT)];
    ...
    [cencel setSize:CGSizeMake(50, 25)];
    [cencel setTitle:[NoticeTools getLocalStrWith:@"main.cancel"] forState:UIControlStateNormal];
    [cencel setTitleColor:[UIColor colorWithHexString:@"#0099E6"] forState:UIControlStateNormal];
    cencel.titleLabel.font = [UIFont systemFontOfSize:16];
    [cencel addTarget:tzImagePickerVc action:@selector(cancelButtonClick) forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:cencel];

    self.navigationController.navigationBar.tintColor = [UIColor colorWithHexString:@"#F7F8FC"];
    UINavigationBar *navgationBar = self.navigationController.navigationBar;
    navgationBar.tintColor = [UIColor colorWithHexString:@"#F7F8FC"];//设置导航条颜色
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:cencel];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:YES completion:^(NSArray<TZAlbumModel *> *models) {
                    self.abumArrary = [NSMutableArray arrayWithArray:models];
                    self->_model = self.abumArrary[0];
                    self->_models = [NSMutableArray arrayWithArray:self->_model.models];
    
                    dispatch_async(dispatch_get_main_queue(), ^{
                        self.albumView.albumArr = self.abumArrary;
                        [self.titleButton setTitle:[NSString stringWithFormat:@"%@   ",models[0].name] forState:UIControlStateNormal];
                    });
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[NoticePhotoListMarkView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header"];
}
...
- (void)configBottomToolBar {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    UINavigationBar *navgationBar = tzImagePickerVc.navigationBar;
    navgationBar.tintColor = [UIColor colorWithHexString:@"#F7F8FC"];//设置导航条颜色
    CGFloat width = DR_SCREEN_WIDTH;
    [navgationBar setBackgroundImage:[UIImage imageFromColor:[UIColor colorWithHexString:@"#F7F8FC"] size:CGSizeMake(width, NAVIGATION_BAR_HEIGHT)] forBarMetrics:UIBarMetricsDefault];//设置背景图片和颜色
    
    [navgationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#25262E"],NSFontAttributeName:[UIFont systemFontOfSize:18]}];//设置文字颜色
    
    self.navigationItem.titleView = self.titleButton;
    
    if (!tzImagePickerVc.showSelectBtn) return;
    
    tzImagePickerVc.view.backgroundColor = [UIColor colorWithHexString:@"#F7F8FC"];
    _bottomToolBar = [[UIView alloc] initWithFrame:CGRectZero];
    //    CGFloat rgb = 253 / 255.0;
    _bottomToolBar.backgroundColor = [UIColor colorWithHexString:@"#F7F8FC"];
    ...
    [_previewButton setTitleColor:[NoticeTools isWhiteTheme] ? GetColorWithName(VDarkTextColor) : [UIColor colorWithHexString:@"3e3e4a"] forState:UIControlStateNormal];
    ...
    //    CGFloat rgb2 = 237 / 255.0;
    _divideLine.backgroundColor = [UIColor clearColor];
    ...
    self.navigationItem.rightBarButtonItems = @[[[UIBarButtonItem alloc]initWithCustomView:_doneButton]];
...
- (void)choiceWith:(TZAlbumModel *)albumM{
    self.isDrop = NO;
    [self.albumView dissMiss:self.titleButton];
    _model = albumM;
...
- (void)closeClick{
    if (!self.isDrop) {
        [self.albumView show:self.titleButton to:self.view];
...
- (FSCustomButton *)titleButton{
    if (!_titleButton) {
        ...
        //[btn setTitle:[NoticeTools getLocalStrWith:@"group.photos"] forState:UIControlStateNormal];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat naviBarHeight = NAVIGATION_BAR_HEIGHT;
    ...
    _doneButton.frame = CGRectMake(self.view.tz_width - _doneButton.tz_width - 12, 0, 78, 25);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section {
    ...
    return CGSizeMake(DR_SCREEN_WIDTH, 30);
}

// 返回Header/Footer内容
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    
    if ([kind isEqualToString:UICollectionElementKindSectionHeader] && indexPath.section == 0) {          // Header视图
        // 从复用队列中获取HooterView
        NoticePhotoListMarkView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header" forIndexPath:indexPath];
        return headerView;
    }
    return nil;
}
...
- (void)refreshBottomToolBarStatus {
    ...
    if (_doneButton.enabled) {
        _doneButton.layer.borderWidth = 0;
    }else{
        _doneButton.layer.borderWidth = 1;
    }
    if (tzImagePickerVc.selectedModels.count) {
        [_doneButton setTitle:[NSString stringWithFormat:@"%@(%zd)",[NoticeTools getLocalStrWith:@"photo.next"],tzImagePickerVc.selectedModels.count] forState:UIControlStateNormal];
        [_doneButton setTitleColor:[UIColor colorWithHexString:@"#FFFFFF"] forState:UIControlStateNormal];
        _doneButton.backgroundColor = [UIColor colorWithHexString:@"#0099E6"];
    }else{
        [_doneButton setTitle:[NoticeTools getLocalStrWith:@"photo.next"] forState:UIControlStateNormal];
        [_doneButton setTitleColor:[UIColor colorWithHexString:@"#FFFFFF"] forState:UIControlStateNormal];
        _doneButton.backgroundColor = [UIColor colorWithHexString:@"#A1A7B3"];
