...
- (void)viewDidLoad {
    ...
    self.navigationBar.tintColor =GetColorWithName(VBackColor);
...
- (void)configDefaultSetting {
    ...
    self.iconThemeColor =GetColorWithName(VBackColor);
...
- (void)configDefaultImageName {
    ...
    self.photoSelImageName = @"selectorImage";
...
- (void)setPhotoDefImageName:(NSString *)photoDefImageName {
    ...
    _photoDefImage = [UIImage imageNamedFromMyBundle:@"photo_def_photoPickerVc"];
}
...
- (void)configDefaultBtnTitle {
    self.doneBtnTitleStr = [NoticeTools getLocalStrWith:@"photo.next"];
    self.cancelBtnTitleStr = [NSBundle tz_localizedStringForKey:@"Cancel"];
    self.previewBtnTitleStr = [NSString stringWithFormat:@"最多可上传%ld张相片",self.maxImagesCount];
    if ([NoticeTools getLocalType] ==1) {
        self.previewBtnTitleStr = [NSString stringWithFormat:@"Maximum %ldphotos",self.maxImagesCount];
    }else if ([NoticeTools getLocalType] ==2){
        self.previewBtnTitleStr = [NSString stringWithFormat:@"最大%ld枚の画像をアップロードできます",self.maxImagesCount];
    }
