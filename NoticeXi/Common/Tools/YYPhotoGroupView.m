...
@interface YYPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate,LCActionSheetDelegate>
...
@property (nonatomic, assign) BOOL navAndToolViewHide;
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    if (self.hideKeybord) {
        self.hideKeybord(YES);
    }
    self.backgroundColor =_isNeedTools? [UIColor blackColor]:[UIColor clearColor];
    ...
    if (_blurEffectBackground && !_isNeedTools) {
    ...
    self.blurBackground.alpha =_isNeedTools?1: 0;
    self.pager.alpha = _isNeedTools?1: 0;
...
- (void)dismiss {
    ...
    if (_isNeedTools) {
        self.navAndToolViewHide = !self.navAndToolViewHide;

        if (self.delegate && [self.delegate respondsToSelector:@selector(tapHideOrShow:)]) {
            [self.delegate tapHideOrShow:self.navAndToolViewHide];
        }
        return;
    }
...
//如果需要工具栏和导航栏
- (void)setIsNeedTools:(BOOL)isNeedTools{
    _isNeedTools = isNeedTools;
    if (isNeedTools) {
...
- (void)setIsSendToFriend:(BOOL)isSendToFriend{
    _isSendToFriend = isSendToFriend;
    if (isSendToFriend) {
        UIButton *sendOrCollBtn = [[UIButton alloc] initWithFrame:CGRectMake((DR_SCREEN_WIDTH-135)/2, _contentView.frame.size.height-65-35,135, 35)];
        [sendOrCollBtn setTitle:[NoticeTools getLocalStrWith:@"hh.givetolike"] forState:UIControlStateNormal];
        [sendOrCollBtn setTitleColor:[UIColor colorWithHexString:@"#FFFFFF"] forState:UIControlStateNormal];
        sendOrCollBtn.titleLabel.font = FOURTHTEENTEXTFONTSIZE;
        sendOrCollBtn.backgroundColor = [UIColor colorWithHexString:@"#1D1E24"];
        [self.contentView addSubview:sendOrCollBtn];
        sendOrCollBtn.layer.cornerRadius = 35/2;
        sendOrCollBtn.layer.masksToBounds = YES;
        [sendOrCollBtn addTarget:self action:@selector(sendOrCollClick) forControlEvents:UIControlEventTouchUpInside];
    }
}

- (void)setIschangeIcon:(BOOL)ischangeIcon{
    _ischangeIcon = ischangeIcon;
    if (ischangeIcon) {
        UIButton *sendOrCollBtn = [[UIButton alloc] initWithFrame:CGRectMake((DR_SCREEN_WIDTH-247)/2, _contentView.frame.size.height-65-35,247, 50)];
        [sendOrCollBtn setTitle:[NoticeTools getLocalStrWith:@"intro.changetx"] forState:UIControlStateNormal];
        [sendOrCollBtn setTitleColor:[UIColor colorWithHexString:@"#FFFFFF"] forState:UIControlStateNormal];
        sendOrCollBtn.titleLabel.font = EIGHTEENTEXTFONTSIZE;
        sendOrCollBtn.backgroundColor = [UIColor colorWithHexString:@"#1D1E24"];
        [self.contentView addSubview:sendOrCollBtn];
        sendOrCollBtn.layer.cornerRadius = 50/2;
        sendOrCollBtn.layer.masksToBounds = YES;
        [sendOrCollBtn addTarget:self action:@selector(chaniconClick) forControlEvents:UIControlEventTouchUpInside];
    }
}
...
- (void)sendOrCollClick{
    ...
    [self removeFromSuperview];
    if (self.sendToFriendBlock) {
        self.sendToFriendBlock(YES,tile.imageView.image);
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_isNeedTools) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(scrollWithModel: saveImage:)]) {
            YYPhotoGroupCell *tile = [self cellForPage:self.currentPage];
            [self.delegate scrollWithModel:self.dataArr[intPage] saveImage:tile.imageView.image];
        }
    }
...
- (void)actionSheet:(LCActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        ...
        if(tile.imageView.image){
            // 保存图片
            UIImageWriteToSavedPhotosAlbum(tile.imageView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        }else{
            [self uploadImageAction:tile.item.largeImageURL];
        }
...
// 下载图片
- (void)uploadImageAction:(NSURL *)imgPath {

    UIImageView *tempIV = [UIImageView new];
    [tempIV sd_setImageWithURL:imgPath completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (image) {
            // 保存图片
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if (error) {
        ...
        [YZC_AlertView showTopWithText:[NoticeTools getLocalStrWith:@"图片保存失败"]];
    } else {
        [YZC_AlertView showTopWithText:[NoticeTools getLocalStrWith:@"yl.baocsus"]];
    }
