...
- (void)initPickerHeaderView {
    ...
    [cancelButton setTitleColor:[UIColor colorWithHexString:@"#A1A7B3"] forState:UIControlStateNormal];
    ...
    [confirmButton setTitleColor:[UIColor colorWithHexString:@"#0099E6"] forState:UIControlStateNormal];
...
- (void)confirm {
    ...
    if (self.manager.datePickerStyle == CXDateYear) {
        if (self.timeBlock) {
            self.timeBlock(self.yearName);
        }
    }else{
        if (self.doneBlock) {
            self.doneBlock(date);
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    switch (self.manager.datePickerStyle) {
        ...
        case CXDateYearMonthDay:
            if (component == 0) {
                title = [NSString stringWithFormat:@"%@年",self.manager.yearArray[row]];
            }
            if (component == 1) {
                title =  [NSString stringWithFormat:@"%@月",self.manager.monthArray[row]];
            }
            if (component == 2) {
                title =  [NSString stringWithFormat:@"%@日",self.manager.dayArray[row]];
            }
            break;
    ...
    switch (self.manager.datePickerStyle) {
        ...
        case CXDateYear:{
            ...
            if (component == 0) {
                self.manager.yearIndex = row;
                self.backYearView.text = self.manager.yearArray[row];
                self.yearName = self.backYearView.text;
            }
